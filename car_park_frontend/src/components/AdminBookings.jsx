        ...
        <table>
            ...
            {bookings.map(b => (
              <tr key={b._id}>
                ...
                <td>
                  {b.paymentSlip ? <a href={`http://localhost:5000/uploads/payments/${b.paymentSlip}`} target="_blank" rel="noreferrer">View</a> : "No Slip"}
                </td>
                <td>{b.qrCode ? <img src={b.qrCode} alt="QR" style={{ width: 50, height: 50 }} /> : "N/A"}</td>
                <td>
                  {b.status.toLowerCase() === "pending" && (
                    <>
                      <button onClick={() => updateStatus(b._id, "approve")}>Approve</button>
                      <button onClick={() => updateStatus(b._id, "reject")} style={{ marginLeft: "5px" }}>Reject</button>
                    </>
                  )}
                  {(b.status.toLowerCase() === "pending" || b.status.toLowerCase() === "approved") && (
                    <button onClick={() => cancelBooking(b._id)} style={{ marginLeft: "5px", backgroundColor: "#6c757d", color: "#fff" }}>
                      Cancel
                    </button>
                  )}
                </td>
